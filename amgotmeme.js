...
myPatreon.style.backgroundSize = "100% 100%";
  myPatreon.setAttribute("class", "Peps&Spax Patreon");
myPatreon.onclick = ()=>{
    window.open("https://www.patreon.com/user?u=78289083");
}
...
document.querySelector("." + name).onclick = ()=>{settings.customSkin = link; connect(settings.server); document.querySelector(".windowclosebtn").click();};
	...
	if (localStorage.getItem("customSkins")){
		...
		const textnode = document.createTextNode("Peps & Spax snay.io script.   ");
...
emojiBindBtns[i].onclick = ()=>{
	...
	var checkEmojiBindInterval = setInterval(()=>{
		...
		logo.src = "https://cdn.discordapp.com/attachments/1110945421586604082/1120861440924995584/NCdpUQE.png";
